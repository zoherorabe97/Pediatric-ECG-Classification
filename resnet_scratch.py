# =============================================================================
# ResNet1D Ablation Study — Model Size × Training Sample Size
# Architecture: SE-ResNet1D in three widths: Large / Medium / Small
#
...
# LOGGING
# =============================================================================
log_file = "logging/child_ecg_ablation_resnet.log"
...
def conv7x1(in_p, out_p, stride=1):
    return nn.Conv1d(in_p, out_p, kernel_size=7, stride=stride, padding=3, bias=False)


def conv1x1(in_p, out_p, stride=1):
    return nn.Conv1d(in_p, out_p, kernel_size=1, stride=stride, bias=False)
...
# THREE-SIZE ResNet1D
# =============================================================================
# Width multipliers per model size:
#   large  → base channels: [64, 128, 256, 512]   (same as original)
#   medium → base channels: [32,  64, 128, 256]
#   small  → base channels: [16,  32,  64, 128]
# =============================================================================
SIZE_CHANNELS = {
...
class ResNet1D(nn.Module):
    """
    Parameterised SE-ResNet18-style 1-D network.

    Args:
        in_channel : input leads (12)
        n_classes  : output classes
        size       : "large" | "medium" | "small"
    """

    def __init__(self, in_channel: int = 12, n_classes: int = 1,
                 size: str = "large"):
        super().__init__()
        ch = SIZE_CHANNELS[size]          # [c1, c2, c3, c4]
        self.inplanes = ch[0]

        # Stem
        self.conv1   = nn.Conv1d(in_channel, ch[0], kernel_size=15,
                                 stride=2, padding=7, bias=False)
        self.bn1     = nn.BatchNorm1d(ch[0])
        self.relu    = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool1d(kernel_size=3, stride=2, padding=1)

        # Residual stages (2 blocks each, mirrors ResNet-18)
        self.layer1 = self._make_layer(BasicBlock, ch[0], blocks=2)
        self.layer2 = self._make_layer(BasicBlock, ch[1], blocks=2, stride=2)
        self.layer3 = self._make_layer(BasicBlock, ch[2], blocks=2, stride=2)
        self.layer4 = self._make_layer(BasicBlock, ch[3], blocks=2, stride=2)

        # Head
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.fc      = nn.Linear(ch[3] * BasicBlock.expansion, n_classes)
...
def run_experiment(
    ...
    """
    Train one ResNet1D variant, evaluate on the test split, return summary dict.
    """
    ...
    # ── model ─────────────────────────────────────────────────────────────────
    set_seeds(SEED)
    model = ResNet1D(in_channel=12, n_classes=n_classes, size=size).to(device)
    total_p, train_p = count_params(model)
    logger.info(f"  Model size      : {size}")
    logger.info(f"  Channels        : {SIZE_CHANNELS[size]}")
...
def main():
    ...
    # ── log model size info ───────────────────────────────────────────────────
    logger.info("\nModel size overview:")
    logger.info(f"  {'Size':<8} {'Channels':<30} {'Total Params':>14} {'Trainable Params':>18}")
    logger.info(f"  {'-'*72}")
    for size in MODEL_SIZES:
        dummy = ResNet1D(12, n_classes, size)
        tp, trp = count_params(dummy)
        logger.info(f"  {size:<8} {str(SIZE_CHANNELS[size]):<30} {tp:>14,} {trp:>18,}")
        del dummy
