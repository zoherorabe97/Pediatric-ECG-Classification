...
# ---------------------------------------------------------------------------
# CI helpers — read pre-computed per-label AUROC_CI and average to macro CI
# ---------------------------------------------------------------------------
def parse_ci_str(s):
    """Parse '(0.8, 0.9)' or '(np.float64(0.8), np.float64(0.9))' -> (low, high)."""
    if pd.isna(s):
...
def macro_ci_from_csv(csv_path, auroc_col="AUROC", ci_col="AUROC_CI"):
    """Load a test_results.csv, average per-label CI bounds -> (macro_low, macro_high)."""
    if not os.path.exists(csv_path):
        return np.nan, np.nan
    df = pd.read_csv(csv_path)
    # normalise column names (case-insensitive)
    col_map = {c.lower(): c for c in df.columns}
    auroc_col = col_map.get(auroc_col.lower(), col_map.get("auroc"))
    ci_col    = col_map.get(ci_col.lower(),    col_map.get("auroc_ci"))
    if auroc_col is None or ci_col is None:
        return np.nan, np.nan
    lows, highs = [], []
    for _, row in df.iterrows():
        lo, hi = parse_ci_str(row[ci_col])
...
# --- Pediatric FT CIs ---
def _ped_ft_ci(key):
    """key is 'zeroshot', 'ft_100', 'ft_500', ... 'ft_full'"""
    sub = "zeroshot" if key == "zeroshot" else key
    f = os.path.join(RES, "ablation_ft", sub, f"{sub}_test_results.csv")
    return macro_ci_from_csv(f)

pz_ci   = _ped_ft_ci("zeroshot")
p100_ci = _ped_ft_ci("ft_100")
p500_ci = _ped_ft_ci("ft_500")
p1k_ci  = _ped_ft_ci("ft_1000")
p3k_ci  = _ped_ft_ci("ft_3000")
pfu_ci  = _ped_ft_ci("ft_full")

# --- ResNet / Net1D scratch CIs (best model size per budget) ---
def best_scratch_exp(df, n_req):
    """Return the experiment_id of the best model for a given training budget."""
    sub = df[df["n_train_requested"].astype(str) == str(n_req)]
    if len(sub) == 0:
        return None
    return sub.loc[sub["test_macro_AUROC"].idxmax(), "experiment"]

def _scratch_ci(df, ablation_dir, n_req):
    exp_id = best_scratch_exp(df, n_req)
    if exp_id is None:
        return np.nan, np.nan
    f = os.path.join(RES, ablation_dir, exp_id, f"{exp_id}_test_results.csv")
    return macro_ci_from_csv(f)

resn_ci  = {n: _scratch_ci(resn,  "ablation_resnet", n) for n in [100, 500, 1000, 3000]}
resn_ci["full"]  = _scratch_ci(resn,  "ablation_resnet", "full")
net1d_ci = {n: _scratch_ci(net1d, "ablation_net1d",  n) for n in [100, 500, 1000, 3000]}
net1d_ci["full"] = _scratch_ci(net1d, "ablation_net1d",  "full")

# --- Adult CinC CIs (zero-shot and full FT per dataset) ---
def _cinc_ci(dataset, mode):
    fname = f"{dataset}_zero_shot_results.csv" if mode == "zero_shot" else f"{dataset}_ft_results.csv"
    f = os.path.join(CINC, fname)
    return macro_ci_from_csv(f, auroc_col="auroc", ci_col="auroc_ci")

cinc_zs_ci = {ds: _cinc_ci(ds, "zero_shot") for ds in PAL}
cinc_ft_ci = {ds: _cinc_ci(ds, "fine_tune")  for ds in PAL}

# Convenience: asymmetric errorbars from a (val, (low,high)) pair
def _eb(val, ci):
    """Return [[err_low], [err_high]] suitable for ax.bar yerr / ax.errorbar."""
    lo, hi = ci
    if np.isnan(lo) or np.isnan(hi):
        return [[0], [0]]
    return [[max(val - lo, 0)], [max(hi - val, 0)]]

def _eb_many(vals, cis):
    """Vectorised _eb for a list of (val, ci) pairs."""
    lows  = [max(v - ci[0], 0) if not np.isnan(ci[0]) else 0 for v, ci in zip(vals, cis)]
    highs = [max(ci[1] - v,  0) if not np.isnan(ci[1]) else 0 for v, ci in zip(vals, cis)]
    return [lows, highs]
...
def plot_1():
    ...
    all_ci = [cinc_zs_ci[d] for d in ds] + [pz_ci]
    all_eb = _eb_many(all_v, all_ci)

    fig, ax = plt.subplots(figsize=(9, 5))
    bars = ax.bar(all_d, all_v, color=all_c, width=0.55, edgecolor="white", linewidth=0.8,
                  yerr=all_eb, capsize=4, error_kw=dict(elinewidth=1.2, ecolor="#333333"))
...
def plot_2():
    ...
    # Pediatric line with CI shading
    ped_xs   = [0,   100,  500,  1000, 3000, n_fu]
    ped_ys   = [pz, p100, p500,  p1k,  p3k,  pfu]
    ped_cis  = [pz_ci, p100_ci, p500_ci, p1k_ci, p3k_ci, pfu_ci]
    ...
    lo = [ci[0] if not np.isnan(ci[0]) else y for y, ci in zip(ped_ys, ped_cis)]
    hi = [ci[1] if not np.isnan(ci[1]) else y for y, ci in zip(ped_ys, ped_cis)]
    ax.fill_between(mapped_ped_xs, lo, hi, color=PC, alpha=0.15, zorder=4)
...
def plot_3():
    ...
    pv_ci = [p100_ci if n==100 else p500_ci if n==500 else p1k_ci if n==1000 else p3k_ci for n in sizes]
    rv_ci = [resn_ci[n] for n in sizes]
    ...
    ax.bar(x + 0.5*w, pv,  w, label="Pediatric ECGFounder FT", color=PC,
           yerr=_eb_many(pv, pv_ci), capsize=3, error_kw=dict(elinewidth=1.2, ecolor="#333333"))
    ax.bar(x + 1.5*w, rv,  w, label="Pediatric ResNet (best)", color="#2ca02c", alpha=0.85,
           yerr=_eb_many(rv, rv_ci), capsize=3, error_kw=dict(elinewidth=1.2, ecolor="#333333"))
...
def plot_4():
    ...
    # Plot FT line with CI shading
    ft_ys  = [pz, p100, p500, p1k, p3k, pfu]
    ft_cis = [pz_ci, p100_ci, p500_ci, p1k_ci, p3k_ci, pfu_ci]
    ax.plot(mapped_xs, ft_ys, marker="D", color=PC, lw=2.5, ms=7,
            label="ECGFounder (Fine-Tuned)", zorder=5)
    ft_lo = [ci[0] if not np.isnan(ci[0]) else y for y, ci in zip(ft_ys, ft_cis)]
    ft_hi = [ci[1] if not np.isnan(ci[1]) else y for y, ci in zip(ft_ys, ft_cis)]
    ax.fill_between(mapped_xs, ft_lo, ft_hi, color=PC, alpha=0.15, zorder=4)
    ...
    for sz in sizes:
        ...
        for n in nreqs:
            ...
            if len(sub):
                exp_id = sub["experiment"].values[0]
                ys.append(sub["test_macro_AUROC"].values[0])
                f = os.path.join(RES, "ablation_resnet", exp_id, f"{exp_id}_test_results.csv")
                ys_ci.append(macro_ci_from_csv(f))
            else:
                ys.append(np.nan)
                ys_ci.append((np.nan, np.nan))
        ...
        sc_lo = [ci[0] if not np.isnan(ci[0]) else y for y, ci in zip(ys, ys_ci)]
        sc_hi = [ci[1] if not np.isnan(ci[1]) else y for y, ci in zip(ys, ys_ci)]
        ax.fill_between(mapped_xs, sc_lo, sc_hi, color=pal2[sz], alpha=0.10)
    ...
    for sz in sizes:
        ...
        for n in nreqs:
            ...
            if len(sub):
                exp_id = sub["experiment"].values[0]
                ys.append(sub["test_macro_AUROC"].values[0])
                f = os.path.join(RES, "ablation_net1d", exp_id, f"{exp_id}_test_results.csv")
                ys_ci.append(macro_ci_from_csv(f))
            else:
                ys.append(np.nan)
                ys_ci.append((np.nan, np.nan))
...
def plot_6():
    ...
    pv_ci  = [p100_ci, p500_ci, p1k_ci, p3k_ci, pfu_ci]
    _ci_key = lambda n: "full" if n == n_fu else n
    rv_ci  = [resn_ci[_ci_key(n)]  for n in sizes]
    nv_ci  = [net1d_ci[_ci_key(n)] for n in sizes]
    ...
    b3 = ax.bar(x + w, nv, w, label="Net1D (best scratch)",  color="#ff7f0e", alpha=0.85, zorder=3,
                yerr=_eb_many(nv, nv_ci), capsize=3, error_kw=dict(elinewidth=1.2, ecolor="#333333"))
...
def plot_8():
    ...
    for idx, ds in enumerate(datasets_order):
        ...
        if ds == "Pediatric":
            ...
            sc_cis = [(0.5,0.5), resn_ci[100], resn_ci[500], resn_ci[1000], resn_ci[3000], resn_ci["full"]]
            zs_val = pz
        else:
            ...
            sc_ys  = [0.5] + list(sub_sc["macro_auroc"])
            ft_cis = None
            sc_cis = None
        ...
        if ft_cis is not None:
            flo = [ci[0] if not np.isnan(ci[0]) else y for y,ci in zip(ft_ys,ft_cis)]
            fhi = [ci[1] if not np.isnan(ci[1]) else y for y,ci in zip(ft_ys,ft_cis)]
            ax.fill_between(ft_xs, flo, fhi, color=ft_color, alpha=0.15)
            slo = [ci[0] if not np.isnan(ci[0]) else y for y,ci in zip(sc_ys,sc_cis)]
            shi = [ci[1] if not np.isnan(ci[1]) else y for y,ci in zip(sc_ys,sc_cis)]
            ax.fill_between(sc_xs, slo, shi, color=sc_color, alpha=0.15)
