# =============================================================================
# Iterative Pruning with Recovery Fine-tuning
# Prunes the fine-tuned ECGFounder model in steps, runs recovery fine-tuning
# after each pruning step to restore performance, then evaluates on full dataset.
# =============================================================================
...
saved_dir  = "./res/iterative_pruning/"
log_file   = "logging/iterative_pruning.log"

# Fine-tuned checkpoint from ft.py
pth = "C:/Users/zoorab/Desktop/zoher/University/Projects/ECGFounder/res/finetune/child_ecg_full_ft_epoch7_auroc0.9072.pth"
...
# =============================================================================
# MODEL LOADER — strips 'backbone.' prefix from ft_ChildECG checkpoints
# =============================================================================
def load_fresh_model(pth, n_classes, device):
    ...
    if has_backbone_prefix:
        state_dict = {k[len("backbone."):]: v for k, v in raw_sd.items() if k.startswith("backbone.")}
...
def plot_roc_curves(all_gt, all_pred, labels, n_classes, split_name, saved_dir):
    ...
    macro_line, = ax.plot(mean_fpr, mean_tpr, color="#f0c040", lw=2.8, ls="--", zorder=10,
                          label=f"Macro-average  (AUC={macro_auc_val:.3f})")
    ax.fill_between(mean_fpr, np.clip(mean_tpr - std_tpr, 0, 1),
                    np.clip(mean_tpr + std_tpr, 0, 1), color="#f0c040", alpha=0.10, zorder=9)
    ...
    micro_line, = ax.plot(fpr_m, tpr_m, color="#40e0d0", lw=2.8, ls="-.", zorder=10,
                          label=f"Micro-average  (AUC={micro_auc_val:.3f})")

    chance_line, = ax.plot([0, 1], [0, 1], color="#666666", lw=1.0, ls=":", zorder=5,
    ...
    ax.set_title(
        ...
        f"{n_valid} valid / {n_classes} total  |  "
    ...
    summary_l = [f"Macro-average  (AUC={macro_auc_val:.3f})", "Macro ± 1 std",
                 f"Micro-average  (AUC={micro_auc_val:.3f})", "Chance  (AUC=0.500)"]
    legend_summary = ax.legend(summary_h, summary_l, loc="lower right", fontsize=10,
    ...
    ax.legend([x[0] for x in class_sorted], [x[1] for x in class_sorted],
...
# =============================================================================
# FULL EVALUATION  — skipped labels logged once at summary level only
# =============================================================================
def full_evaluation(all_gt, all_pred, labels, n_classes, split_name, saved_dir, n_resamples=100):
    ...
    # per-class AUROC table (valid labels only)
    logger.info(f"\n{'='*72}")
    ...
    # skipped labels: count only, no individual listing
    logger.info(f"\n[{split_name}] Valid labels : {len(results_df)} / {n_classes}")
    logger.info(f"[{split_name}] Skipped      : {len(skipped_df)} / {n_classes}  (all-zero, saved to {skipped_path})")
...
# =============================================================================
def recovery_finetune(model, trainloader, device, n_epochs, lr, wd, split_name):
    """Fine-tune the pruned model for a few epochs to recover lost performance."""
    ...
    logger.info(f"  Recovery fine-tuning: {n_epochs} epochs | lr={lr} | wd={wd}")

    for epoch in range(n_epochs):
        model.train()
        train_loss = 0.0
        for x, y in tqdm(trainloader, desc=f"  Recovery [{split_name}] epoch {epoch+1}/{n_epochs}", leave=False):
            x, y = x.to(device), y.to(device)
            optimizer.zero_grad()
        ...
        logger.info(f"  Recovery epoch {epoch+1}/{n_epochs} | Loss: {avg_loss:.4f} | LR: {current_lr:.2e}")
...
def main():
    ...
    # ── train / eval split (patient-level, same seed as ft.py) ───────────────
    ...
    logger.info(f"Recovery train samples : {len(train_df)}")
    logger.info(f"Full eval samples      : {len(ecg_df)}")

    # ── dataloaders ───────────────────────────────────────────────────────────
    train_dataset = ECG_Dataset(ecg_path=ecg_path, df=train_df, target_fs=target_fs)
    ...
    if sanity_macro < 0.55:
        raise RuntimeError(
            ...
            "Verify that 'pth' points to an ft.py checkpoint."
        )
    ...
    for ratio in PRUNING_RATIOS:
        logger.info("\n" + "=" * 60)
        logger.info(f"PRUNING RATIO {ratio}  |  method: {PRUNING_METHOD}  |  recovery: {RECOVERY_EPOCHS} epochs")
        logger.info("=" * 60)

        # fresh copy of fine-tuned model — each ratio is independent
        ...
        model = recovery_finetune(
            model       = model,
            trainloader = trainloader,
            device      = device,
            n_epochs    = RECOVERY_EPOCHS,
            lr          = RECOVERY_LR,
            wd          = RECOVERY_WD,
        ...
        # save recovered pruned model
        ckpt_path = os.path.join(
            saved_dir,
            f"pruned_{PRUNING_METHOD}_ratio{ratio}"
            f"_recovery{RECOVERY_EPOCHS}ep"
            f"_auroc{macro_auroc:.4f}.pth"
        )
        model.zero_grad()
        torch.save({
            "model"           : model,
    ...
    summary_path = os.path.join(saved_dir, "iterative_pruning_summary.csv")
